...
uses
  ...
  procedure DrawRGB32Bitmap(Dest: HDC; DstX, DstY: Integer; SrcX, SrcY, SrcWidth, SrcHeight: Integer; Bitmap: TRGB32BitmapCore);
  //procedure DrawRGB32BitmapA(Dest: HDC; DstX, DstY: Integer; SrcX, SrcY, SrcWidth, SrcHeight: Integer; Bitmap: TRGB32BitmapCore);
...
begin
  ...
  begin
    ...
    {PD := Dst.GetPixelPtrUnsafe(X, Y + I);
    for J := 0 to SrcWidth-1 do begin
      PBYTE(PD + (J shl 2) + 0)^ := $ff;
      PBYTE(PD + (J shl 2) + 1)^ := $ff;
      PBYTE(PD + (J shl 2) + 2)^ := $ff;
      PBYTE(PD + (J shl 2) + 3)^ := $ff;
    end; }
  end;
end;
  ...
  if ( turn ) then begin
    ...
    for I := 0 to Pred(SrcHeight) do begin
      for J := 0 to Pred(SrcWidth) do begin
        PS := Src.GetPixelPtrUnsafe(SrcX + J, SrcY + I);
        PD := Dst.GetPixelPtrUnsafe( X + SrcHeight - I - 1, Y + J );
        Move(PS^, PD^, 4);
      end;
    end;
...
begin
 ...
 {
procedure DrawRGB32BitmapAA(Dst: TRGB32BitmapCore; X, Y: Integer; Src: TRGB32BitmapCore);
var
  SrcX, SrcWidth, SrcY, SrcHeight: Integer;
  I: Integer;
  PS, PD: PRGB32Pixel;
...
// ! SrcX < 0, SrcY < 0, SrcX + SrcWidth > Bitmap.Width, SrcY + SrcHeight > Bitmap.Height
// ! results in mash
procedure DrawRGB32BitmapA(Dest: HDC; DstX, DstY: Integer; SrcX, SrcY, SrcWidth, SrcHeight: Integer;
  Bitmap: TRGB32BitmapCore);
var
  Clip: TRect;
{$IFDEF Win32}
...
{$UNDEF StretchRGB32}
{$ENDIF}
procedure StretchDrawRGB32Bitmap(Dest: HDC; DstX, DstY, DstWidth, DstHeight: Integer; SrcX, SrcY, SrcWidth, SrcHeight: Integer; Bitmap: TRGB32BitmapCore);
var
  Clip: TRect;
{$IFDEF StretchRGB32}
  {$IFDEF gtk}
