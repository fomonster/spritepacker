  ...
  if not showAlpha then begin
    ...
    for y:=picture.bitmap.Height-1 downto 0 do begin
      for x:=0 to picture.bitmap.Width-1 do begin
         c:=picture.bitmap.Get32PixelUnsafe(x, y);
         c := c or $ff000000;
         picture.bitmap.Set32PixelUnsafe(x, y, c);
      end;
    end;
...
procedure TMainForm.FixHolesAlpha(bitmap:TRGB32Bitmap; _x:Integer; _y:Integer; _trimX:Integer; _trimY:Integer; _width:Integer; _height:Integer; turn:Boolean);
  var
   ...
   begin
     ...
     c:=bitmap.Get32PixelUnsafe(fromX, fromY);

     ca := c and $ff000000;
     c:=bitmap.Get32PixelUnsafe(toX, toY);
     c := (c and $00ffffff) or ca;
     bitmap.Set32PixelUnsafe(toX, toY, c);
   end;
...
procedure TMainForm.FixHoles(srcBitmap:TRGB32Bitmap);
  ...
  begin
    ...
    if ( y > 0 ) then begin
      if ( x > 0 ) then c7:=bitmap.Get32PixelUnsafe(x-1, y-1);
      c8:=bitmap.Get32PixelUnsafe(x, y-1);
      if ( x < bitmap.Width-1 ) then c9:=bitmap.Get32PixelUnsafe(x+1, y-1);
    end;

    if ( x > 0 ) then c4:=bitmap.Get32PixelUnsafe(x-1, y);
    c5:=bitmap.Get32PixelUnsafe(x, y);
    if ( x < bitmap.Width-1 ) then c6:=bitmap.Get32PixelUnsafe(x+1, y);

    if ( y < bitmap.Height-1 ) then begin
      if ( x > 0 ) then c1:=bitmap.Get32PixelUnsafe(x-1, y+1);
      c2:=bitmap.Get32PixelUnsafe(x, y+1);
      if ( x < bitmap.Width-1 ) then c3:=bitmap.Get32PixelUnsafe(x+1, y+1);
    end;
    ...
    dstBitmap.Set32PixelUnsafe(x, y, c5);
  end;
...
begin
  ...
  for y:=srcBitmap.Height-1 downto 0 do begin
    for x:=0 to srcBitmap.Width-1 do begin
      c:=newBitmap.Get32PixelUnsafe(x, y);
